...
import {
    ...
    TrendingUp, Hash, Calculator, Lock, AlertCircle
} from 'lucide-react';
import {
...
const TABLE_LABELS: Record<string, string> = {
    ...
    crm_opportunities: 'Deals & Opportunities',
    vw_crm_opportunities: 'Deals & Opportunities',
    crm_deals: 'Deals & Opportunities',
    crm_leads: 'Leads & Prospects',
    vw_crm_leads: 'Leads & Prospects',
    crm_customers: 'Customers & Contacts',
    vw_crm_customers: 'Customers & Contacts',
...
export const ReportBuilder: React.FC<ReportBuilderProps> = ({ onBack, companyId, initialModule, editReport, moduleFilter }) => {
    ...
    const resolveSourceTable = (table: string): string => {
        if (table === 'crm_deals' || table === 'crm_opportunities' || table === 'vw_crm_opportunities') return 'vw_crm_opportunities';
        if (table === 'crm_leads' || table === 'vw_crm_leads') return 'vw_crm_leads';
        if (table === 'customers' || table === 'crm_customers' || table === 'vw_crm_customers') return 'vw_crm_customers';
        return table;
    };
    ...
    const handlePreview = async () => {
        ...
        setQueryError(null);
        try {
            const queryTable = resolveSourceTable(sourceTable);
            const selectStr = selectedColumns.map(c => c.field.field_key).join(',');
            // @ts-ignore
            let query: any = (supabase as any).from(queryTable).select(selectStr, { count: 'exact' });
            if (companyId) query = query.eq('company_id', companyId);

            // Row-level private view filter when !isAdmin
            if (!isAdmin && user?.id) {
                const empId = await getLinkedEmployeeId(user.id);
                if (queryTable === 'vw_crm_leads' || sourceTable === 'crm_leads') {
                ...
                } else if (queryTable === 'vw_crm_opportunities' || sourceTable === 'crm_deals' || sourceTable === 'crm_opportunities') {
                    const conds = [`created_by.eq.${user.id}`, `owner_id.eq.${user.id}`];
                    if (empId) conds.push(`owner_id.eq.${empId}`, `created_by.eq.${empId}`);
                    query = query.or(conds.join(','));
                } else if (queryTable === 'vw_crm_customers' || sourceTable === 'customers' || sourceTable === 'crm_customers') {
                    const conds = [`created_by.eq.${user.id}`, `owner_id.eq.${user.id}`];
                    if (empId) conds.push(`owner_id.eq.${empId}`, `created_by.eq.${empId}`);
                    query = query.or(conds.join(','));
            ...
            setQueryError(err?.message || 'Failed to fetch report data. Please check your selected columns and filters.');
            setPreviewData([]);
            setHasRun(true);
        } finally {
            setLoading(false);
                    ...
                    <div ref={printRef} className="flex-1 overflow-auto p-6">
                        {selectedColumns.length === 0 ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center gap-3">
                                ...
                                <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Generating report preview...</p>
                            </div>
                        ) : queryError ? (
                            <div className="h-full flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto">
                                ...
                                <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 mb-1">Unable to Load Report</h3>
                                <p className="text-xs text-rose-600 dark:text-rose-400 mb-4">{queryError}</p>
                                <button onClick={handlePreview} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-colors flex items-center gap-1.5 shadow-sm">
                            ...
                            <div className="h-full flex flex-col items-center justify-center">
                                <button
                                    ...
                                    className="flex flex-col items-center justify-center p-8 rounded-2xl hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 border-2 border-dashed border-slate-200 dark:border-zinc-800 hover:border-indigo-300 dark:hover:border-indigo-600 transition-all cursor-pointer group max-w-sm"
                                >
                                    <div className="w-16 h-16 bg-indigo-50 dark:bg-indigo-900/20 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-indigo-100 dark:group-hover:bg-indigo-900/40 transition-all shadow-sm">
                                        <Play className="w-8 h-8 text-indigo-500 fill-indigo-500/20 group-hover:fill-indigo-500 transition-colors" />
                                    </div>
                                    <h3 className="text-base font-bold text-slate-700 dark:text-slate-200 mb-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                        Ready to Run Preview
                                    </h3>
                                    <p className="text-xs text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors text-center">
                                        Click here or the <strong>Run</strong> button in the top bar to preview your report data
                                    </p>
                                </button>
                            </div>
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-slate-400">
                                ...
                                <h3 className="text-base font-bold text-slate-600 dark:text-slate-300 mb-1">No Records Found</h3>
                                <p className="text-xs text-slate-400">No data matched the selected criteria or filters.</p>
                            </div>
                        ) : viewMode === 'TABLE' ? (
