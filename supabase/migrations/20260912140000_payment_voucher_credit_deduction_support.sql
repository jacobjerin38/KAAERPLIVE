-- Migration: 20260912140000_payment_voucher_credit_deduction_support.sql
-- Description: Enable credit entries / deductions in payment vouchers (e.g. income allocations, employee advance recovery, contra expenses)
        ...
        IF v_has_multi_expense THEN
            ...
            LOOP
                ...
                IF v_line_acc_id IS NOT NULL AND v_line_amount <> 0 THEN
                    ...
                    -- Post as CREDIT to that account (e.g. Rental Income, Advance Recovery, Contra Expense)
                    IF v_line_amount < 0 OR v_line_entry_type = 'credit' THEN
                        ...
                        VALUES (v_company_id, v_entry_id, v_line_acc_id, COALESCE(v_line_partner_id, v_payment.partner_id), v_line_memo, 0, ABS(v_line_amount));
                    ELSE
                        -- Normal Debit line (Expense, Asset, Liability)
                        INSERT INTO public.accounting_journal_lines (company_id, entry_id, account_id, partner_id, name, debit, credit)
                        VALUES (v_company_id, v_entry_id, v_line_acc_id, COALESCE(v_line_partner_id, v_payment.partner_id), v_line_memo, ABS(v_line_amount), 0);
                    END IF;
                END IF;
            END LOOP;
        ELSE
    ...
    ELSE
        ...
        IF v_has_multi_expense THEN
            ...
            LOOP
                ...
                IF v_line_acc_id IS NOT NULL AND v_line_amount <> 0 THEN
                    -- If signed negative OR explicitly marked as debit deduction on receipt:
                    IF v_line_amount < 0 OR v_line_entry_type = 'debit' THEN
                        INSERT INTO public.accounting_journal_lines (company_id, entry_id, account_id, partner_id, name, debit, credit)
                        VALUES (v_company_id, v_entry_id, v_line_acc_id, COALESCE(v_line_partner_id, v_payment.partner_id), v_line_memo, ABS(v_line_amount), 0);
                    ELSE
                        -- Normal Credit line on Receipt (Revenue, Customer Receivable credit)
                        INSERT INTO public.accounting_journal_lines (company_id, entry_id, account_id, partner_id, name, debit, credit)
                        VALUES (v_company_id, v_entry_id, v_line_acc_id, COALESCE(v_line_partner_id, v_payment.partner_id), v_line_memo, 0, ABS(v_line_amount));
                    END IF;
                END IF;
