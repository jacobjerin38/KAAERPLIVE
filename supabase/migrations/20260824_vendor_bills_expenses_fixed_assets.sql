-- ==============================================================================
-- Migration: 20260824_vendor_bills_expenses_fixed_assets.sql
-- Description: Update rpc_create_accounting_invoice and rpc_update_accounting_invoice
--              to support direct Expense and Fixed Asset account selection (account_id)
--              in Vendor Bills alongside Item purchases and Purchase Ledgers.
-- ==============================================================================
...
BEGIN
    ...
    -- 1. Get Partner Details & AR/AP Account in the active accounting_chart_of_accounts
    ...
    IF p_move_type = 'out_invoice' THEN
        -- Direct check on partner's assigned receivable account
        ...
        -- Fallback to 1110 Sundry Debtors in company
        ...
        -- Fallback to any active posting Receivable / Asset account
        IF v_receivable_payable_account_id IS NULL THEN
            SELECT id INTO v_receivable_payable_account_id
            FROM public.accounting_chart_of_accounts
            WHERE company_id = v_company_id 
    ...
    ELSIF p_move_type = 'in_invoice' THEN
        -- Direct check on partner's assigned payable account
        ...
        -- Fallback to 2010 Sundry Creditors or 2001 Accounts Payable
        IF v_receivable_payable_account_id IS NULL THEN
            SELECT id INTO v_receivable_payable_account_id
            FROM public.accounting_chart_of_accounts
            WHERE company_id = v_company_id 
        ...
        -- Fallback to any active posting Payable / Liability account
        IF v_receivable_payable_account_id IS NULL THEN
            SELECT id INTO v_receivable_payable_account_id
            FROM public.accounting_chart_of_accounts
            WHERE company_id = v_company_id 
    ...
    -- 2. Create Header (Draft State) in accounting_journal_entries
    ...
    LOOP
        ...
        -- Step A: Direct Account ID passed (for Expense, Fixed Asset, or COA account)
