-- Migration: 20260917140000_fix_receipt_and_partner_payment_posting.sql
-- Description: Fix receipt voucher (money in) debit/credit balancing and partner payment account resolution in rpc_post_accounting_payment.
-- Live Data Safety: 100% Backward Compatible. Preserves all existing payments, balances, and single/multi-line postings.
...
BEGIN
            ...
            LOOP
                ...
                IF v_line_acc_id IS NOT NULL AND v_line_amount <> 0 THEN
                    ...
                    IF v_line_amount < 0 OR v_line_entry_type = 'credit' THEN
                        ...
                        VALUES (v_company_id, v_entry_id, v_line_acc_id, v_line_partner_id, v_line_memo, 0, ABS(v_line_amount));
                    ELSE
                        -- Normal Debit line (Expense, Asset, Liability, Vendor Payable)
                        INSERT INTO public.accounting_journal_lines (company_id, entry_id, account_id, partner_id, name, debit, credit)
                        VALUES (v_company_id, v_entry_id, v_line_acc_id, v_line_partner_id, v_line_memo, ABS(v_line_amount), 0);
                    END IF;
                END IF;
            END LOOP;
        ELSE
            ...
            END IF;
            INSERT INTO public.accounting_journal_lines (company_id, entry_id, account_id, partner_id, name, debit, credit)
            VALUES (v_company_id, v_entry_id, v_counterpart_account_id, v_payment.partner_id, v_line_name, ABS(v_payment.amount), 0);
        END IF;
    ...
    ELSE
        ...
        ELSE
            ...
            END IF;
            INSERT INTO public.accounting_journal_lines (company_id, entry_id, account_id, partner_id, name, debit, credit)
            VALUES (v_company_id, v_entry_id, v_counterpart_account_id, v_payment.partner_id, v_line_name, 0, ABS(v_payment.amount));
        END IF;
