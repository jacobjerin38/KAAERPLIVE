-- ====================================================================
-- Migration: 20260815_harden_accounting_rpcs_and_isolation.sql
-- Description: Hardens invoice/bill RPCs with company isolation,
--              direct accounting_chart_of_accounts resolution,
--              posting account validation, and safe foreign key constraints.
-- ====================================================================

-- 1. Harden rpc_create_accounting_invoice
...
BEGIN
    ...
    IF p_move_type = 'out_invoice' THEN
        -- A. Direct check on partner's assigned receivable account
        ...
        -- B. Fallback to 1110 Sundry Debtors in company
        ...
        -- C. Fallback to any active posting Receivable / Asset account
        IF v_receivable_payable_account_id IS NULL THEN
            SELECT id INTO v_receivable_payable_account_id
            FROM public.accounting_chart_of_accounts
            WHERE company_id = v_company_id 
    ...
    ELSIF p_move_type = 'in_invoice' THEN
        -- A. Direct check on partner's assigned payable account
        ...
        -- B. Fallback to 2010 Sundry Creditors or 2001 Accounts Payable
        IF v_receivable_payable_account_id IS NULL THEN
            SELECT id INTO v_receivable_payable_account_id
            FROM public.accounting_chart_of_accounts
            WHERE company_id = v_company_id 
        ...
        -- C. Fallback to any active posting Payable / Liability account
        IF v_receivable_payable_account_id IS NULL THEN
            SELECT id INTO v_receivable_payable_account_id
            FROM public.accounting_chart_of_accounts
            WHERE company_id = v_company_id 
    ...
    LOOP
        ...
        -- A. Resolve ledger account directly if specified
        ...
        -- C. Fallback to standard company Income or Expense posting accounts
        ...
        -- Override narration if passed explicitly
...
-- 2. Harden rpc_update_accounting_invoice
...
BEGIN
    ...
    LOOP
        ...
        -- Fallback to standard posting accounts
        IF v_account_id IS NULL THEN
            IF v_move_type = 'out_invoice' THEN
                SELECT id INTO v_account_id
                FROM public.accounting_chart_of_accounts
